...
{
    const auto player = mManager->FindUnique<PlayerComponent>();

    const auto& transform = mManager->GetComponent<TransformComponent>(player);
    ...
    auto projection = fra::ProjectionUniformBuffer {
        ...
        .projection = glm::perspective(
            glm::radians(60.0f),
            static_cast<float>(mWindow->GetWidth()) / static_cast<float>(mWindow->GetHeight()), 0.1f,
