...
class LaserGunSystem : public fr::System
{
  public:
    LaserGunSystem(const Ref<fr::Scene>&         scene,
                   ...
                   const Ref<OctreeSystem>
                       octreeSystem) :
        System(scene), mOctreeSystem(octreeSystem)
    {
        mBulletModel =
            meshPool->CreateMeshFromFile("./Resources/Models/moon.fbx");
        mBulletTexture = texturePool->CreateTextureFromFile(
            "./Resources/Textures/blank_texture.png");
        mBulletMaterial = materialPool->Create(
            { mBulletTexture, mBulletTexture, mBulletTexture });

        mScene->AddEventListener<CollisionEvent>([this](const CollisionEvent event) {
            OnCollision(event);
        });
    }
