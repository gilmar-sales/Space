    ...
    mScene->CreateQuery()->EachAsync<LaserGunComponent, TransformComponent, RigidBodyComponent, SquadComponent>(
        [this, deltaTime](auto entity,
            ...
            if (laserGun.triggered) {
                ...
                const auto material = squad.squad == Squad::Ally
                                          ? mAssetManager->GetBulletMaterial()
                                          : mAssetManager->GetEnemyBulletMaterial();

                Shoot(entity, material, transform.position + leftOffset,
                      transform.rotation,
                      rigidBody.velocity + transform.GetForwardDirection() * 500.f);
                Shoot(entity, material, transform.position + rightOffset,
                      transform.rotation,
                      rigidBody.velocity + transform.GetForwardDirection() * 500.f);
            }
        });
                        ...
                        [&](const TransformComponent &transform,
                            ...
                            mScene->CreateArchetypeBuilder()
                                    ...
                                    .ForEach<TransformComponent,
                                        ...
                                        ModelComponent>(
                                        [this, velocity = rigidBody.mass / count, radius = sphereCollider.radius,
                                            maxSize](
                                    auto entity,
