...
void LaserGunSystem::Shoot(fr::Entity owner, glm::vec3 position, glm::vec3 velocity)
{
    mScene->CreateEntity(
        ...
        TransformComponent { .position = position, .rotation = glm::vec3(0.0), .scale = glm::vec3(1.0) },
        SphereColliderComponent { .radius = 1.0f },
        ModelComponent { .meshes = &mAssetManager->GetBulletModel(), .material = mAssetManager->GetBulletMaterial() },
